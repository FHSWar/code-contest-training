...
4. **`<=` vs `<` 的选择**：取决于命中时能不能立刻收工。
   - **`left <= right` + 提前返回**：找到一个就够了，不关心是不是"最优"（704、33、74）。
   - **`left < right` + 收敛到底**：找到了还想看看有没有更好的——最左、最小、第一个满足条件的（34、35、153、162、540、875、1283、1482、378、1011、410）。
...
6. **位运算速记**：`mid & 1` 取最低位（奇 1 偶 0），`mid -= mid & 1` 强制偶数。比 `mid % 2` 快且常用于消除奇偶分支（如 540）。
